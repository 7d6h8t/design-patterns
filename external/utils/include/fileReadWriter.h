...
class fileReadWriter final {
   public:
    ...
    static std::vector<uint8_t> readFileToBinary(const path_t& filePath) {
        std::basic_ifstream<uint8_t> file(filePath,
    ...
    template <typename path_t>
    static void writeFileFromBinary(const path_t& filePath,
                                    const std::vector<uint8_t>& buffer) {
        std::basic_ofstream<uint8_t> file(filePath, std::ios_base::out |
                                                        std::ios_base::out |
                                                        std::ios::binary);
    ...
    static str_t readFileToString(const path_t& filePath) {
        ...
        if (false == readFileToString(filePath, contents))
            throw std::logic_error("fileToString Error");
    ...
    static bool readFileToString(const path_t& filePath, str_t& contents) {
        ...
        contents.assign((std::istream_iterator<char_type>(ReadFile)),
    ...
    template <typename path_t,
              typename str_t,
              typename char_type = typename str_t::value_type>
    static bool writeStringToFile(const path_t& filePath,
                                  const str_t& contents) {
        // SMB(server)상의 파일 Write 시에 Read | Write 옵션으로 createFile
        // 호출해야 더빠르다. test시 대략 3배정도 빠름.
    ...
    template <typename path_t,
              typename str_t,
              typename char_type = typename str_t::value_type>
    static bool writeStringToFile(const path_t& temp_dir,
        ...
        temp_stream.flush();
        BOOL copy_ret =
            ::CopyFileW(reinterpret_cast<LPCWSTR>(temp_path.c_str()),
