...
Frame I420Converter::GetFrame(const BYTE* frame_data, const int32_t pitch,
                              const SIZE& size) {
...
void I420Converter::SaveFrame(std::ofstream& file_stream,
                              const Frame& frame) {
...
I420Converter::CreateInputputShaderResource(const Frame& frame) {
  ...
  const BYTE* u_bits = frame.bits_.get() + (frame.pitch_ * frame.size_.cy);
